...
//        assertTrue("Size of paused trigger groups list expected to be 0 ", pausedGroups.size() == 0);
        ...
        assertFalse("Scheduler should have returned 'false' from attempt to unschedule non-existing trigger. ", sched.unscheduleJob(key("foasldfksajdflk")));

        assertTrue("Scheduler should have returned 'true' from attempt to unschedule existing trigger. ", sched.unscheduleJob(key("t3"/* , "g1"  */)));
...
//                    .storeDurably()
            ...
            try {
                ...
                assertFalse("Unexpected existence of job named 'j2'.", sched.checkExists(key("j2")));
            }

            sched.addJob(nonDurableJob, false, true);

            assertTrue("Unexpected non-existence of job named 'j2'.", sched.checkExists(key("j2")));
        } finally {
            sched.shutdown(true);
        }
    }
