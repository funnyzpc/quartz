...
//    }
    ...
    public void testTriggerOperations() throws Exception {
        Key triggerKey = new Key(TRIGGER_KEY/* , GROUP_KEY */);
        GroupMatcher<Key<?>> groupMatcher = GroupMatcher.triggerGroupEquals(null/*GROUP_KEY*/);
        ...
        try {
            remoteScheduler.getTriggersOfJob(new Key(JOB_KEY/* , GROUP_KEY */));
            fail("Method had a different return type in MBean API");
        } catch (SchedulerException e) {
            // expected
        }
...
//        assertThat(scheduler.getTriggerState(triggerKey), is(Trigger.TriggerState.NORMAL));
    ...
    public void testJobOperations() throws Exception {
        ...
        Key job2 = new Key("job2"/* , GROUP_KEY */);
        JobDetail job2Detail = newJob(HelloJob.class).withIdentity(job2)
//                .storeDurably()
                .build();
...
//        GroupMatcher<JobKey> matcher = GroupMatcher.jobGroupEquals(null/*GROUP_KEY*/);
...
//        }
        ...
        assertThat(remoteScheduler.unscheduleJob(Key.key(TRIGGER_KEY/* , GROUP_KEY */)), is(true));
        try {
        ...
        try {
//            remoteScheduler.deleteJobs(Collections.singletonList(JobKey(JOB_KEY/* , GROUP_KEY */)));
            remoteScheduler.deleteJobs(Collections.singletonList(Key.key(JOB_KEY/* , GROUP_KEY */)));
            fail("Method was not exposed in MBean API");
        } catch (SchedulerException e) {
            // expected
        }
...
//        assertThat(remoteScheduler.getJobGroupNames().isEmpty(), is(true));
    ...
    public static class TestRemoteScheduler extends RemoteMBeanScheduler {
        ...
        @Override
        public void pauseJobs(String triggerName) throws SchedulerException {

        }

        @Override
        public Set<Key> getAllJobKeysInSched(String triggerName) throws SchedulerException {
            return null;
        }
    }
}
