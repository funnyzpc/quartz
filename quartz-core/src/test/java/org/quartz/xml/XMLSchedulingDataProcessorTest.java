...
//			assertEquals(1, scheduler.getAllJobKeysInSched("DEFAULT").size());
////			assertEquals(1, scheduler.getTriggerKeys(GroupMatcher.triggerGroupEquals("DEFAULT")).size());
//
//			job = scheduler.getJobDetail(Key.key("job1"));
...
//			assertEquals(2, scheduler.getAllJobKeysInSched("DEFAULT").size());
...
//
//			// then
//			Trigger trigger = scheduler.getTrigger(new Key("job1"/*, "DEFAULT"*/));
...
//
//			trigger = scheduler.getTrigger(new Key("job2"/*, "DEFAULT"*/));
//			assertNotNull(trigger);
//
...
//
////			final JobKey jobKey = scheduler.getAllJobKeysInSched("native").iterator().next();
