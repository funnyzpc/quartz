...
//        validateState();
////        OperableTrigger trig = (OperableTrigger) newTrigger().withIdentity(newTriggerId()).forJob(jobKey).build();
//        OperableTrigger trig = (OperableTrigger) newTrigger().withIdentity(newTriggerId()).withIdentity(jobKey).build();
...
//    }
    ...
    public boolean checkExists(Key jobKey) throws SchedulerException {
        validateState();
        return resources.getJobStore().checkTriggerExists(jobKey);
    }
