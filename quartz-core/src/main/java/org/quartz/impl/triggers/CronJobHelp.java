 ...
 */
public class CronJobHelp extends AbstractTrigger<CronTrigger> implements CronTrigger/*, CoreTrigger*/ {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(CronJobHelp.class);
    ...
    @Override
    public Object clone() {
        CronJobHelp copy = (CronJobHelp) super.clone();
        if (cronEx != null) {
    ...
    public void setStartTime(Date startTime) {
        ...
        // round off millisecond...  四舍五入毫秒。。。
        // Note timeZone is not needed here as parameter for 注意，此处不需要时区作为参数
        // Calendar.getInstance(),
        // since time zone is implicit when using a Date in the setTime method. 因为在setTime方法中使用Date时，时区是隐式的。
...
//     * CronTrigger的扩展使用它来暗示还有其他属性，特别是这样扩展就可以选择是存储为序列化blob还是扁平CronTrigger表。
//     */
//    @Override
