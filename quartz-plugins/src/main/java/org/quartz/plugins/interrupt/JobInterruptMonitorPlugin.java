...
public class JobInterruptMonitorPlugin extends TriggerListenerSupport implements SchedulerPlugin {
    ...
    @SuppressWarnings("rawtypes")
    public ScheduledFuture scheduleJobInterruptMonitor(Key jobkey, long delay) {
    ...
    static class InterruptMonitor implements Runnable {
        ...
        InterruptMonitor(Key jobKey, Scheduler scheduler) {
            this.jobKey = jobKey;
            this.scheduler = scheduler;
        }
        ...
        public void run() {
            try {
                ...
                getLog().info("Interrupting Job as it ran more than the configured max time. Job Details [" + jobKey.getName()+"]");
